...
if __name__ == "__main__":
    ...
    df = clean_supply_delivery_data("supply_delivery_history.csv")
    ...
    if df is not None:
        ...
        top_vendors_by_records(df, 10)

        shipment_mode_summary(df)

        product_group_summary(df)

        country_product_group_summary(df)

        shipment_mode_by_country(df)

        fig = plot_bar_chart(top_countries, "Top 10 Countries by Shipment Records")
        plt.show()

    else:
        print("Data could not be loaded. Please check your CSV file path.")